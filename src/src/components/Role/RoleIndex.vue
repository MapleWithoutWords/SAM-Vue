<template>
  <div>
    ...
    <el-breadcrumb>
      ...
      <el-breadcrumb-item>应用系统角色</el-breadcrumb-item>
    </el-breadcrumb>
    <!--主体-->
    <el-card>
      <!--搜索-->
      ...
      <el-table :data="moduleData" :stripe="true" :border="true">
        ...
        <el-table-column prop="childCount" label="子角色数"></el-table-column>
          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                icon="el-icon-files"
                @click="
                  RoleModuleDialog.visible = true
                  RoleModuleDialog.authGroupId = scope.row.id
                  RoleModuleDialog.title = `模块授权-【${queryInfo.appName}】-【${scope.row.name}】`
                "
                circle
              ></el-button>
            </el-tooltip>
    ...
    >
      ...
      >
        ...
        <el-form-item label="角色名" :required="true" prop="name">
        ...
        <el-form-item label="描述" prop="remark">
          <el-input
            type="textarea"
            v-model="createOrEdirotDialog.form.remark"
          ></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog
      :close-on-click-modal="false"
      :title="RoleModuleDialog.dialogTitle"
      :visible.sync="RoleModuleDialog.visible"
    >
      <sam-role-auth
  ...
  methods: {
    ...
    async loadData() {
      ...
      var res = await this.$http({
        url: '/api/PmsRole/getbyurl?' + query,
        method: 'get'
      })
    ...
    // 创建
    createData() {
      this.createOrEdirotDialog.dialogTitle = '新增角色'
    ...
    async editorData(index, data) {
      var editorData = await this.$sendAsync({
        url: '/api/PmsRole/getbyid/' + data.id,
        method: 'get'
      })
      if (editorData === null) {
        return false
      }
      this.createOrEdirotDialog.dialogTitle = '编辑角色'
    ...
    async delData(index, data) {
      var resData = await this.$delById(`/api/PmsRole/delete/${data.id}`)
      if (resData !== true) {
        return false
      }
    ...
    addChild(data) {
      ...
      console.log(this.queryInfo.parentId)
    },
    // 回到上一级
    backUp() {
