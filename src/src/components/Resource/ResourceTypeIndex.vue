<template>
  <div>
    ...
    <el-breadcrumb>
      ...
      <el-breadcrumb-item>资源类型</el-breadcrumb-item>
    </el-breadcrumb>
    <!--主体-->
    <el-card>
      <!--搜索-->
    ...
    >
      ...
      >
        <el-form-item label="资源类型名" :required="true" prop="name">
  ...
  methods: {
    ...
    async loadData() {
      ...
      var res = await this.$http({
        url: '/api/pmsresourcetype/getbyurl?' + query,
        method: 'get'
      })
      ...
      this.$refs.form.validate(async valid => {
        ...
        if (this.createOrEdirotDialog.isAdd === false) {
          var resData = await this.$sendAsync({
            url: '/api/pmsresourcetype/addnew',
            method: 'put',
          ...
          var editorResData = await this.$sendAsync({
            url: '/api/pmsresourcetype/update',
            method: 'post',
            data: that.createOrEdirotDialog.form
          })
    ...
    async delData(index, data) {
      var resData = await this.$delById(
        `/api/pmsresourcetype/delete/${data.id}`
      )
      if (resData !== true) {
        return false
      }
