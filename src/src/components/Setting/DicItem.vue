  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="appData" :stripe="true" :border="true">
        ...
        <el-table-column prop="otherCode" label="OtherCode"></el-table-column>
        <!--操作列-->
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              v-show="!queryInfo.parentId"
              size="mini"
              icon="el-icon-aim"
    ...
    >
      ...
      >
        ...
        <el-form-item label="Code" :required="true" prop="code">
          <el-input v-model="createOrEdirotDialog.form.code"></el-input>
        </el-form-item>
        <el-form-item label="其他Code" prop="otherCode">
          <el-input v-model="createOrEdirotDialog.form.otherCode"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    async loadData() {
      ...
      var res = await this.$http({
        url: '/api/PmsDicItem/getbyurl?' + query,
        method: 'get'
      })
    ...
    // 创建
    createData() {
      this.createOrEdirotDialog.dialogTitle = '新增配置'
    ...
    editorData(index, data) {
      this.createOrEdirotDialog.dialogTitle = '编辑配置'
      ...
      this.$refs.form.validate(async valid => {
        ...
        if (this.createOrEdirotDialog.isAdd === false) {
          ...
          if (resData !== null) {
            ...
            that.queryInfo.count += 1
          }
        } else {
          var editorResData = await this.$sendAsync({
            url: '/api/PmsDicItem/update',
            method: 'post',
            data: that.createOrEdirotDialog.form
          })
    ...
    async delData(index, data) {
      var resData = await this.$delById(`/api/PmsDicItem/delete/${data.id}`)
      if (resData !== true) {
        return false
      }
      this.appData = this.appData.filter(e => e.id !== data.id)
      this.queryInfo.count += 1
